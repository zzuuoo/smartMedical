...
//        String date = df.format(new Date(medicalRecord.getAdmissionTime()));// new Date()为获取当前系统时间，也可使用当前时间戳
        ...
        if(medicalRecord.getPatient().getName()!=null&&!medicalRecord.getPatient().getName().equals("")){
            holder.medical_record_patient_name.setText(medicalRecord.getPatient().getName());
        }else{
            holder.medical_record_patient_name.setText(medicalRecord.getPatient().getAccount());
        }
